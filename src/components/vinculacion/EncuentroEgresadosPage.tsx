// src/components/vinculacion/EncuentrosEgresadosPage.tsx
...
import {
  getEgresadosResources,
  uploadEgresadosResource,
...
// Modal para capturar título/descripcion de imagen
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-xl bg-white rounded-2xl shadow-lg p-6">
        <h3 className="text-lg font-semibold mb-4">Metadatos de la imagen</h3>
...
const EncuentrosEgresadosPage: React.FC = () => {
  ...
  const loadResources = async () => {
    try {
      const resources: any[] = await getEgresadosResources();
      ...
      console.error("Error loading recursos egresados:", error);
  ...
  const handleImageMetaConfirm = async (title: string, description: string) => {
    ...
    try {
      ...
      await uploadEgresadosResource(form);
  ...
  const handleDeleteImageConfirmed = async (id: string) => {
    ...
    setIsSaving(true);
    try {
      await deleteEgresadosResource(parseInt(id));
  ...
  const handlePdfView = (pdf: PdfItem) => setSelectedPdfForView(pdf);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto font-sans text-gray-800">
      <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8 border-b pb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Encuentros de Egresados</h1>
          <p className="text-sm text-gray-500 mt-2">Gestiona imágenes y documentos relacionados a los encuentros de egresados.</p>
        </div>
                ...
                {images.map((img) => (
                  <div key={img.id} className="group border rounded-lg overflow-hidden bg-white hover:shadow-md transition-shadow relative">
                    <div className="h-40 bg-gray-100 overflow-hidden relative cursor-pointer" onClick={() => setViewImageUrl(img.url)}>
                      ...
                      <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><Eye className="text-white drop-shadow-md" size={24} /></div>
                    </div>
                    ...
                    <div className="p-3">
                      ...
                      <div className="flex items-center justify-end gap-2 mt-3 pt-2 border-t border-gray-100">
                        <button onClick={() => handleImageDownload(img)} className="p-1.5 text-gray-500 hover:text-green-600 hover:bg-green-50 rounded transition-colors" title="Descargar"><Download size={16} /></button>
                        <button onClick={() => setConfirmDeleteImageId(img.id)} className="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded transition-colors" title="Eliminar"><Trash2 size={16} /></button>
                      </div>
                    </div>
                  </div>
                ))}
...
export default EncuentrosEgresadosPage;

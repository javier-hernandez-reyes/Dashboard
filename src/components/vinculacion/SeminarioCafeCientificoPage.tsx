// src/components/vinculacion/SeminarioCafeCientificoPage.tsx
...
import { 
  getSeminarioResources, 
  uploadSeminarioResource, 
  deleteSeminarioResource 
} from "../../services/seminarioCafeService";
...
const SeminarioCafeCientificoPage: React.FC = () => {
  ...
  const loadResources = async () => {
    try {
      const resources = await getSeminarioResources();
      
      // Process PDF (take the latest one)
      const pdfs = resources.filter(r => r.tipo === 'pdf');
      if (pdfs.length > 0) {
      ...
      // Process Images
      const imgs = resources.filter(r => r.tipo === 'image').map(r => ({
        id: r.id.toString(),
        title: r.titulo,
  ...
  const onPdfPicked: React.ChangeEventHandler<HTMLInputElement> = async (e) => {
    ...
    try {
      ...
      await uploadSeminarioResource(formData);
  ...
  const handlePdfDownload = () => {
    ...
    a.href = pdfUrl;
    a.download = pdfFileName ?? "seminario_cafe.pdf";
    a.target = "_blank"; // Open in new tab if download fails or for preview
  ...
  const handlePdfDeleteConfirmed = async () => {
    ...
    setIsSaving(true);
    try {
      await deleteSeminarioResource(pdfId);
  ...
  const handleDeleteImageConfirmed = async (id: string) => {
    ...
    setIsSaving(true);
    try {
      await deleteSeminarioResource(parseInt(id));
  ...
  const handleSaveAll = () => {
    // Since we are saving immediately on upload/delete, this button might be redundant
    // or could be used to refresh data.
    loadResources();
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto font-sans text-gray-800">
      ...
      <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8 border-b pb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Seminario Café Científico</h1>
          <p className="text-sm text-gray-500 mt-2">Gestiona el documento informativo y la galería de eventos del seminario.</p>
        </div>
...
export default SeminarioCafeCientificoPage;